...
class SynthMMHS(data.Dataset):
    ...
    def __init__(self, text_field, label_field, id_field, path=None, split_name = None, train_vocab_path = None, examples=None, **kwargs):
        ...
        print("Num split samples: " + str(len(examples)))
    ...
    def splits(cls, text_field, label_field, id_field, split_folder, split_name, shuffle=True ,root='.', **kwargs):
        ...
        Arguments:
            ...
            Remaining keyword arguments: Passed to the splits method of
                Dataset.
        """
        path = "../../../datasets/HateSPic/SynthMMHS/anns/" + split_folder + "/"
        print("Split:  "  + path)
        ...
        fields = [('text', text_field), ('label', label_field), ('id', id_field)]
        train_examples = []
        train_path = "../../../datasets/HateSPic/SynthMMHS/anns/"

        with codecs.open(os.path.join(train_path, 'anns_train.txt'), 'r', 'utf8') as f:
            train_examples += [
                data.Example.fromlist([line.split(',')[2], '0', line.split(',')[0]], fields) for line in f]
        with codecs.open(os.path.join(train_path, 'anns_val.txt'), 'r', 'utf8') as f:
            train_examples += [
                data.Example.fromlist([line.split(',')[2], '0', line.split(',')[0]], fields) for line in f]
...
# load ED dataset
def load_SynthMMHS(text_field, label_field, id_field, batch_size = 1, split_folder='test', split_name = 'tweets.test'):
    print('loading data')
    train_data, test_data = SynthMMHS.splits(text_field, label_field, id_field, split_folder, split_name)
    text_field.build_vocab(train_data, train_data)
    label_field.build_vocab(train_data, train_data)
    id_field.build_vocab(train_data, train_data) # Added but not sure what is doing
